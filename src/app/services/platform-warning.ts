...
const NON_SAFARI_BROWSERS =
  /Chrome|Chromium|CriOS|Edg|EdgiOS|OPR|OPiOS|Firefox|FxiOS|SamsungBrowser|DuckDuckGo|YaBrowser/i;

export function shouldShowApplePlatformWarning(
  platformInfo: BrowserPlatformInfo = navigator,
): boolean {
  ...
  const isSafari =
    /Safari/i.test(userAgent) && /Apple/i.test(vendor) && !NON_SAFARI_BROWSERS.test(userAgent);
